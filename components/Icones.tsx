...
/* ⚠ ESTE É O ÚNICO ÍCONE SÓLIDO DA FILA, e é decisão, não descuido.

   Facebook, Instagram e Local são contorno no gabarito `tracado` abaixo. O
   WhatsApp fica preenchido porque a marca só é RECONHECÍVEL assim: o que
   identifica o símbolo é o par balão-mais-fone, e a 40px, com traço de 2, o
   fone vira borrão dentro do balão. Um balão de contorno vazio leria como
   "mensagem", não como WhatsApp.

   ══ E NÃO PRECISA DE COMPENSAÇÃO DE PESO ══

   A suposição era que um glifo cheio ia pesar mais que um contorno de 2 no
   mesmo quadrado, e ia pedir uma caixa menor no CSS. MEDIDO na capa, fração
   de tinta no quadrado de 40px:

     Instagram   29,5%
     Local       24,6%
     WhatsApp    22,4%   ← sólido, e ainda assim o segundo mais leve
     Facebook    18,8%

   O motivo é a forma da marca: o balão é um ANEL e o fone é pequeno, então o
   glifo é quase todo miolo vazado. Ele cai dentro da faixa dos três contornos
   sem ajuste nenhum, e por isso o CSS trata os quatro igual — `.hero__social
   svg` continua com um tamanho só. */
export function IconeWhatsApp({ className }: PropsIcone) {
  return (
