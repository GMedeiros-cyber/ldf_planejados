...
/* Fundo da seção Processo: shader "Flow field" do 21st.dev nas cores da marca.

   O GLSL abaixo é o original, intocado. Mudou o invólucro e quatro pontos:

   a) cursorEnabled: false. Com true, os cinco listeners de ponteiro entram —
      e um deles é "scroll" em fase de CAPTURA chamando getBoundingClientRect
      a cada evento. Numa página que já tem scroll-driven animation no próprio
      Processo e na navbar, isso é layout thrashing. O efeito fica atrás do
      texto e do véu de qualquer jeito. Os cinco já nascem atrás do mesmo
      `if (UNIFORMS.cursorEnabled)`, então desligar a chave basta.
   b) prefers-reduced-motion impede a criação do contexto — não esconde o
      canvas depois. Mesmo padrão do FundoAuralis.
   c) Paleta LDF: de --ground-deep ao vermelho da marca, com dois marrons no
      meio que já existem no gradiente da História. hue vai a 0, porque o
      4.6251 do original girava as cores antigas.
   d) getContext devolvendo null cai no gradiente estático, não em canvas
      vazio — é o mesmo caminho do (b).

   O canvas nasce em JS, não no JSX, e é descartado no cleanup. O motivo está
   no FundoAuralis e vale aqui também: loseContext() mata o elemento para
   sempre, então reaproveitar o mesmo canvas entre montagens — StrictMode em
   dev, navegação client-side em produção — devolveria um contexto já perdido.
   Com canvas novo a cada montagem, o WeakMap de liberação adiada do original
...
/* (c) Paleta LDF, em RGB normalizado de 0 a 1.
   #0A0909 (--ground-deep) · #2A1410 e #5E2A18, os mesmos marrons do gradiente
   da História · #DF0100 (--red). As quatro últimas posições repetem o vermelho
   só para preencher o array de 8 que o shader declara; colorCount continua 4. */
const UNIFORMS = {
  colors: [
    [0.0392156862745098, 0.03529411764705882, 0.03529411764705882],
    [0.16470588235294117, 0.0784313725490196, 0.06274509803921569],
    [0.3686274509803922, 0.16470588235294117, 0.09411764705882353],
    [0.8745098039215686, 0.00392156862745098, 0],
    [0.8745098039215686, 0.00392156862745098, 0],
    [0.8745098039215686, 0.00392156862745098, 0],
    [0.8745098039215686, 0.00392156862745098, 0],
    [0.8745098039215686, 0.00392156862745098, 0],
  ...
  brightness: 0.07,
  saturation: 1.54,
  hue: 0 /* (c) era 4.6251, calibrado para o índigo do original. */,
  vignette: 0.36,
  blur: 0.0048,
  grain: 0.021,
  seed: 8379.0,
  ...
  offsetY: 0.15,
  drift: 0.06,
  cursorEnabled: false /* (a) desliga os cinco listeners de ponteiro. */,
  cursorEffect: 4.0,
  cursorStrength: 0.66,
  cursorRadius: 0.65,
  oklab: 0.0,
...
export default function FundoShader() {
  ...
  useEffect(() => {
    ...
    /* (b) O objetivo é não criar o contexto, não escondê-lo depois. */
    if (window.matchMedia("(prefers-reduced-motion: reduce)").matches) return;

    const canvas = document.createElement("canvas");
    canvas.className = "fundo-shader__tela";
    canvas.setAttribute("aria-hidden", "true");

    const gl = canvas.getContext("webgl", { antialias: false });
    if (!gl) return; /* (d) Sem contexto: fica o gradiente estático. */
    ...
    /* (a) Com cursorEnabled false, nenhum destes cinco chega a ser registrado:
       pointermove, pointercancel, scroll (captura), blur e pointerleave. */
    ...
    /* Arrow, não `function`: numa declaração hoisted o TypeScript zera o
       narrowing de `gl` feito lá em cima pelo `if (!gl) return`. */
    ...
    return () => {
      ...
      /* O canvas é descartado junto, então loseContext pode ser imediato: não
         sobra elemento reaproveitável para herdar o contexto morto. */
